	...
	public function load_modal_on_footer() {
		...
		while ( $arpc_query->have_posts() ) {
			...
			if ( is_page( $show_in_id ) ) {
				?>
				<div class="arpc-popup-creator arpc-template arpc-<?php echo esc_attr( $template ); ?>" id="arpc-popup-creator" data-auto-hide="<?php echo ( 1 === $auto_hide ) ? 'yes' : 'no'; ?>" data-id="popup-<?php echo get_the_ID(); ?>" data-popup-image-size="<?php echo esc_attr( $image_size ); ?>" data-exit="<?php echo esc_attr( $exit ); ?>" data-delay="<?php echo $delay; ?>" data-show="<?php echo $show_in_id; ?>" data-page="<?php echo $slug; ?>">
